...
function adapterContract(create: () => Adapter) {
  describe('Adapter contract', () => {
    ...
    it('creates and deletes a session', async () => {
      ...
      const s = await adapter.createSession({ userId: u.id, expiresAt: new Date(Date.now() + 60_000) });
    ...
    it('respects token expiry', async () => {
      await adapter.createVerificationToken({ id: 'vt-2', identifier: 'x', token: 't', expiresAt: new Date(Date.now() - 1) });
